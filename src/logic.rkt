...
[define (replace index ele lista)
  (cond((<= (length lista) index) lista)
       ((equal? index 0) (append (list ele) (rest lista)))
        (else (append (list (car lista)) (replace (- index 1) ele (rest lista)))))]
...
[define (winners game)
  (append (result "Croupier" (add_cards (first game))) (winners_aux (first game) (cdr game)))  
  ]
[define (winners_aux crupie players)
  (cond ((null? players) '())
        ...
        ((blackjack? crupie) (append (list "lost to a Croupier BlackJack") (winners_aux crupie (rest players))))
        ...
        ((tie? crupie (first players)) (append (result "Tied" (add_cards (first players))) (winners_aux crupie (rest players))))
...
;Función para saber si el jugador perdió
...
[define (lost? crupier player)
  (cond((or (and (not (keep_playing? player))
       ...
       (and (keep_playing? crupier)
            (< (add_cards player) (add_cards crupier)))) #t)
  (else #f))
  
  ]
;Función para saber si el jugador empató con el crupier
;Input: Una lista con las cartas del crupier y una lista con las cartas del jugador
;Output: #t si hubo un empate y #f de lo contrario
[define (tie? crupier player)
  (cond((= (add_cards player) (add_cards crupier)) #t)
  (else #f))
  ]

;Función para saber si el jugador ganó
;Input: Una lista con las cartas del crupier y una lista con las cartas del jugador
;Output: #t si el jugador ganó y #f de lo contrario
[define (won? crupier player)
  (cond((and (not (keep_playing? crupier)) (not (equal? (add_cards crupier) 21))) #t)
       (else #f))
  ]
...
[define (crupier deck)
  (crupier_aux (first (draw_cards 2 deck)) (second (draw_cards 2 deck)))
  ]
...
[define (draw_cards_aux num deck)
  (cond ((equal? num 0) (list deck)) 
        (else (append (list (first deck)) (draw_cards_aux (- num 1) (rest deck)))
              ))]
...
[define (replace_as_value hand)
  (cond ((null? hand) '())
        ((as? (first hand)) (append (list 11.1) (rest hand)))
        (else (append (list (first hand)) (replace_as_value (rest hand)))))
...
[define (blackjack? cards)
  (cond((and (equal? (length cards) 2)
             (or (and (ten? (first cards)) (as? (second cards)))
                 (and (as? (first cards)) (ten? (second cards))))) #t)
       (else #f))
  ]
...
[define (add_cards_aux hand)  
  (cond((null? hand) 0)
      (else (+ (get_card_value (first hand)) (add_cards_aux (rest hand)))))
  ]
